...
"""
for original pointnet training
"""
...
"""
for pointnet2 part seg
"""

def get_model2(point_cloud, is_training, weight_decay=0, part_num=50, bn_decay=None):
    """ ConvNet baseline, input is BxNx3 gray image """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    end_points = {}
    ...
    with tf.variable_scope('transform_net1') as sc:
        K = 6
        point_cloud_expanded = tf.expand_dims(point_cloud, [2])
        transform = get_transform_K(point_cloud_expanded, is_training, bn_decay, K = 6)
    #point_cloud_transformed = tf.matmul(point_cloud, transform)
    input_image = tf.matmul(point_cloud, transform)
    input_image = tf.expand_dims(input_image, [2])

    #input_image = tf.expand_dims(point_cloud_transformed, -1)
    out1 = tf_util.conv2d(input_image, 64, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv1', bn_decay=bn_decay)
    out2 = tf_util.conv2d(out1, 128, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv2', bn_decay=bn_decay)
    out3 = tf_util.conv2d(out2, 128, [1,1], padding='VALID', stride=[1,1],
...
def get_loss2(seg_pred, seg, end_points):
